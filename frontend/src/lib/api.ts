...
export async function updateRouteControlPoints(
  ...
  const response = await api.put(`/api/routes/${id}/control-points`, {
    control_points: data.controlPoints,
...
export async function joinEditingSession(
  ...
  user_id: string;
  started_at: string;
  active_sessions: EditingSession[];
}> {
  const response = await api.post(
    `/api/routes/${routeId}/editing-session`,
    data,
  );
  return response.data;
}
...
export async function createPointChange(
  ...
  const response = await api.post(`/api/routes/${routeId}/point-changes`, {
    feature_index: data.featureIndex,
    point_index: data.pointIndex,
...
export async function updatePointChangeStatus(
  ...
  const response = await api.patch(`/api/point-changes/${changeId}`, {
    status,
  });
  return response.data;
}
